...
static bool is_client_connected_(NetServer* server, NetClientId id);
...
static void recv_loop_(NetServer* server)
{
    ...
    while (server->recv_loop_status == LS_RUNNING)
    {
        ...
        if (size >= (int)sizeof(NetPacket))
        {
            ...
            case NPT_SYNC:
            {
                if (is_client_connected_(server, p->head.sender))
                {
...
static bool is_client_connected_(NetServer* server, NetClientId id)
{
    return id < server->info.max_clients && server->clients[id].is_connected;
}
...
bool net_server_is_client_connected(NetServer* server, NetClientId id)
{
    ...
    return server->clients[id].is_connected;
}
