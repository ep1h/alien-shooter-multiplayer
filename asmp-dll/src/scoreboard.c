...
Scoreboard* scoreboard_create(int entries_number)
{
    ...
    if (scoreboard)
    {
        ...
        if (scoreboard->entries)
        {
            ...
            scoreboard->w = 382;
            scoreboard->h = 300;
            scoreboard->x = (game_get_render()->width - scoreboard->w) * 0.5f;
            scoreboard->y = (game_get_render()->height - scoreboard->h) * 0.5f;

            scoreboard->header_nickname =
                (EntText*)game_get_game()->__vftable->create_entity(
                    game_get_game(), 0,
                    game_get_game()->vids[VID_004_MENU_FONT_SMALL], 100, 100,
                    1.0f, 0, 0);
...
void scoreboard_draw(Scoreboard* scoreboard)
{
    ...
    /* Draw background */
    Render__draw_colored_rect(game_get_render(), 0, scoreboard->x,
                              scoreboard->y, scoreboard->x + scoreboard->w,
                              scoreboard->y + scoreboard->h, 0x99000000);
    /* Draw header line */
    Render__draw_colored_rect(
        game_get_render(), 0, scoreboard->x + 5, scoreboard->y + 20,
    ...
    Render__draw_colored_rect(game_get_render(), 0, scoreboard->x,
                              ...
                              scoreboard->y + scoreboard->h, 0xFFA89DAE);
    Render__draw_colored_rect(game_get_render(), 0,
